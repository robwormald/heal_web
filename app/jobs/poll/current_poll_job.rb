class Poll::CurrentPollJob < ApplicationJob
  ...
  def perform(user_id, poll_id)
    ...
    if poll
      ...
      ActionCable.server.broadcast("poll_#{user_id}", ChannelHelpers.params(:current_poll, { poll: poll, questions: questions, answered: answer }))
    else
      ActionCable.server.broadcast("poll_#{user_id}", ChannelHelpers.params(:current_poll, {}))
    end
  end
end
